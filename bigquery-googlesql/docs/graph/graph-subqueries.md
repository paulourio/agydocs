...
| Name | Summary |
|---|---|
| [`ARRAY` subquery](#array_subquery) | Subquery expression that produces an array. |
| [`EXISTS` subquery](#exists_subquery) | Checks if a subquery produces at least one row. |
| [`IN` subquery](#in_subquery) | Checks if a subquery produces a specified value. |
| [`VALUE` subquery](#array_subquery) | Subquery expression that produces a scalar value. |
...
- `graph_name`: The name of the property graph.
- `gql_query_expr`: A GQL query expression.
- `match_statement`: A pattern matching operation to perform on a graph. For more information, see [`MATCH` statement](graph-query-statements.md#gql_match).
- `graph_pattern`: A pattern to match in a graph. For more information, see [graph pattern definition](graph-patterns.md#graph_pattern_definition).
