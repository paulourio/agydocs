...
| Name | Summary |
|---|---|
| [`CREATE PROPERTY GRAPH` statement](#gql_create_graph) | Creates a property graph. |
| [`DROP PROPERTY GRAPH` statement](#gql_drop_graph) | Deletes a property graph. |
...
> [!NOTE]
> **Note:** all GQL examples in the GQL reference use the [`FinGraph`](#fin_graph) property graph example. To set up this property graph, see [Create and query a graph](https://docs.cloud.google.com/bigquery/docs/graph-create).
...
- `label_and_properties_list`: The list of labels and properties to add to
  ...
  [Label and properties list definition](#label_property_definition).
...
- `element_properties`: The properties associated with a label. A property
  ...
  [Element properties definition](#element_table_property_definition).
...
- `derived_property`: An expression that defines a property and can optionally
  ...
  - `measure_expression`: An aggregation expression in the format
    ...
    A [measure](../standard-sql/data-types.md#measure_type) uses the `KEY` of the
    ...
    using a measure. You can use the [`GRAPH_EXPAND` TVF](graph-sql-queries.md#graph_expand) to transform your graph into a flattened table that lets you access the measure by calling it with the
    [`AGG` function](../standard-sql/aggregate_functions.md#agg). For limitations on the types of graphs
    that you can use with `GRAPH_EXPAND`, read the
    [input limitations](graph-sql-queries.md#input_limitations). For more information, see
...
Once the property graph is created, you can use it in [GQL](graph-query-statements.md) queries. For
