...
| Name | Summary |
|---|---|
| [Graph pattern](#graph_pattern_definition) | A pattern to search for in a graph. |
| [Element pattern](#element_pattern_definition) | Represents a node pattern or an edge pattern in a path pattern. |
| [Subpath pattern](#graph_subpaths) | Matches a portion of a path. |
| [Quantified path pattern](#quantified_paths) | A path pattern with a portion that can repeat within a specified range. |
| [Label expression](#label_expression_definition) | An expression composed from one or more graph label names in an element pattern. |
| [Path search prefix](#search_prefix) | Restricts path pattern to return all paths, any path, or a shortest path from each data partition. |
| [Path mode](#path_mode) | Includes or excludes paths that have repeating edges. |
...
- `path_search_prefix`: a qualifier for a path pattern to return all paths, any
  path, or any shortest path. For more information, see [Path search prefix](#search_prefix).

- `path_mode`: The [path mode](#path_mode) for a path pattern. Used to filter out
...
- `path_term`: An [element pattern](#element_pattern_definition) or a
  [subpath pattern](#graph_subpaths) in a path pattern.

- `subpath_pattern`: A path pattern enclosed in parentheses. To learn
  more, see [Graph subpath pattern](#graph_subpaths).

- `quantified_path_primary`: The quantified path pattern to add to the
  graph query. To learn more, see [Quantified path pattern](#quantified_paths).
...
- `where_clause`: A `WHERE` clause, which filters the matched results. For
  ...
  graph-specific [predicates](../standard-sql/operators.md#graph_predicates) and
  [logical operators](../standard-sql/operators.md#graph_logical_operators). Use the
  [field access operator](../standard-sql/operators.md#field_access_operator) to access graph properties.
...
- `label_expression`: The expression for the label. For more information,
  see [Label expression definition](#label_expression_definition).

- `where_clause`: A `WHERE` clause, which filters the nodes or edges that were
  matched.

  Boolean expressions are supported, including graph-specific [predicates](../standard-sql/operators.md#graph_predicates)
  and [logical operators](../standard-sql/operators.md#graph_logical_operators).

  The `WHERE` clause can't reference properties when the graph pattern variable
  is absent.

  Use the [field access operator](../standard-sql/operators.md#field_access_operator) to access
  graph properties.
...
Graph elements can be used in GQL [predicates](../standard-sql/operators.md#graph_predicates), [functions](graph-gql-functions.md)
and subqueries within GQL.

Set operations support graph elements that have a common [supertype](../standard-sql/conversion_rules.md#supertypes).
...
variable using [horizontal aggregation](graph-gql-functions.md#gql-horiz-agg-func-calls).
...
- `or_expression`: [GQL logical `OR` operation](../standard-sql/operators.md#graph_logical_operators) for
...
- `and_expression`: [GQL logical `AND` operation](../standard-sql/operators.md#graph_logical_operators) for
  label expressions. For example:
...
- `not_expression`: [GQL logical `NOT` operation](../standard-sql/operators.md#graph_logical_operators) for
  label expressions. For example:
...
in the [`FinGraph`](graph-schema-statements.md#fin_graph) property graph.
...
or an `Account` label in the [`FinGraph`](graph-schema-statements.md#fin_graph) property graph.
...
- `<expr>` must be a [numeric](../standard-sql/data-types.md#numeric_types) type
...
All rules for [quantified path patterns](#quantified_paths) apply. In the
...
A path can have either a path mode or a [path search prefix](#search_prefix), but
not both.
...
[path search prefix](#search_prefix):
...
[path search prefixes](#search_prefix) when the path mode is placed on a subpath.
