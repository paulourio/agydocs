...
use the
[`AI.FORECAST` function](bigqueryml-syntax-ai-forecast.md)
...
- `forecast_timestamp` always has a type of `TIMESTAMP`, regardless of the type of the column specified in the [`TIME_SERIES_TIMESTAMP_COL` option](bigqueryml-syntax-create-time-series.md#time_series_timestamp_col) of the `CREATE MODEL` statement.
...
we recommend that you use the [forecast with limit](bigqueryml-syntax-create-time-series.md#forecast_limit_lower_bound) option instead, because the algorithm it uses is less likely to cause an issue. If you keep getting out of memory errors, you can try working around this issue by
