...
- For time series data, use one of the following models:
  - [`ARIMA_PLUS`](bigqueryml-syntax-create-time-series.md)
  - [`ARIMA_PLUS_XREG`](bigqueryml-syntax-create-multivariate-time-series.md)
- For [independent and identically distributed random variables (IID)](https://en.wikipedia.org/wiki/Independent_and_identically_distributed_random_variables) data, use one of the following models:
  - [K-means models](bigqueryml-syntax-create-kmeans.md)
  - [Principal component analysis (PCA) models](bigqueryml-syntax-create-pca.md)
  - [Autoencoder models](bigqueryml-syntax-create-autoencoder.md)

If you don't want to manage your own times series anomaly detection model, you can
use the
[`AI.DETECT_ANOMALIES` function](bigqueryml-syntax-ai-detect-anomalies.md)
...
- `QUERY_STATEMENT`: The GoogleSQL query that generates the data to use to perform anomaly detection. For the supported SQL syntax for the `QUERY_STATEMENT` clause in GoogleSQL, see [Query syntax](../standard-sql/query-syntax.md#sql_syntax).
...
- `CONTAMINATION`: a `FLOAT64` value that identifies the
  ...
  [K-means model output](#k-means_model_output).
...
- To detect anomalies in historical time-series data, the [`DECOMPOSE_TIME_SERIES`](bigqueryml-syntax-create-time-series.md#decompose_time_series) training option must be set as its default value of `TRUE` when the input model is created. Neither `table_name` nor `query_statement` is accepted.
...
[`TIME_SERIES_TIMESTAMP_COL`](bigqueryml-syntax-create-time-series.md#time_series_timestamp_col)
and
[`TIME_SERIES_DATA_COL`](bigqueryml-syntax-create-time-series.md#time_series_data_col).
...
- The column names of the input data from either the `table` or the `query_statement` argument must match the column names of the model. The column data types must be compatible according to BigQuery [implicit coercion rules](../standard-sql/conversion_rules.md#supertypes).
- If you used the [`TRANSFORM` clause](bigqueryml-syntax-create.md#transform) in the `CREATE MODEL` statement that created the model, then only the input columns present in the `TRANSFORM` clause can appear in `query_statement`.
...
- The function returns `NULL` values in the `is_anomaly`, `upper_bound`, `lower_bound` and `anomaly_probability` columns for rows with invalid input, which include the following cases:
  ...
  - The value in the `TIME_SERIES_TIMESTAMP_COL` column is not in the range of the forecast [horizon](bigqueryml-syntax-create-time-series.md#horizon).
  - The value in the `TIME_SERIES_TIMESTAMP_COL` column does not follow the same [frequency](bigqueryml-syntax-create-time-series.md#data_frequency) as the one in the model.
...
[`DECOMPOSE_TIME_SERIES`](bigqueryml-syntax-create-time-series.md#decompose_time_series)
