...
- `hparam_tuning_evaluation_metrics`: a `STRUCT` value that contains the evaluation metrics appropriate to the hyperparameter tuning objective specified by the [`hparam_tuning_objectives` argument](bigqueryml-syntax-create.md#hparam_tuning_objectives) in the `CREATE MODEL` statement. Metrics are calculated from the evaluation data. For more information about the datasets used in hyperparameter tuning, see [Data split](https://docs.cloud.google.com/bigquery/docs/hp-tuning-overview#data_split).
