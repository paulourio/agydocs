...
function that lets you create embeddings that describe an entity---for example,
...
- To generate embeddings from text or visual content,
  ...
  The `AI.GENERATE_EMBEDDING` function works with the Agent Platform
...
models, see [Supported text embedding models](#supported-text-models).
...
- `MODEL_NAME`: the name of
  ...
  supported [Agent Platform-hosted
  open model](#vertex-open-models)
  or a
  [self-deployed open model](bigqueryml-syntax-create-remote-model-open.md).
...
- `QUERY_STATEMENT`: the GoogleSQL query
  ...
  - If you are creating embeddings from a standard table, the query must
    ...
    - For visual content embeddings, you can provide an
      [`ObjectRef` value](../standard-sql/objectref_functions.md)
...
- `embedding`: an `ARRAY<FLOAT>` value that
  ...
  [`NUM_PRINCIPAL_COMPONENTS` option](bigqueryml-syntax-create-pca.md#num_principal_components)
  value if that option is used when the model is created. If the
  [`PCA_EXPLAINED_VARIANCE_RATIO` option](bigqueryml-syntax-create-pca.md#pca_explained_variance_ratio) is used
...
- `embedding`: an `ARRAY<FLOAT>` value that
  ...
  [`HIDDEN_UNITS` option](bigqueryml-syntax-create-autoencoder.md#hidden_units)
  array value.
...
- `embedding`: an `ARRAY<FLOAT>` value that
  ...
  model's
  [`NUM_FACTORS` option](bigqueryml-syntax-create-matrix-factorization.md#num_factors)
  value.
...
- `processed_input`: a `STRING` value that contains the name of the user or
  ...
  [`query_statement` clause](bigqueryml-syntax-create.md#query_statement)
...
To increase quota, first request more quota for the Agent Platform
