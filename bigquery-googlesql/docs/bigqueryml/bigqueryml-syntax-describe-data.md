...
- Columns of the following data types are [cast](../standard-sql/conversion_functions.md#cast) to `STRING` and return the same statistics as `STRING` columns:
  - `BOOL`
  - `BYTE`
  - `DATE`
  - `DATETIME`
  - `TIME`
  - `TIMESTAMP` Columns of the following data types are [cast](../standard-sql/conversion_functions.md#cast) to `FLOAT64` and return the same statistics as `FLOAT64` columns:
  - `INT64`
  - `NUMERIC`
  - `BIGNUMERIC`
...
- `min`: a `STRING` column that contains the [`MIN`](../standard-sql/aggregate_functions.md#min) value for the column.
- `max`: a `STRING` column that contains the [`MAX`](../standard-sql/aggregate_functions.md#max) value for the column.
...
- `median`: a `FLOAT64` column that contains one of the following:
  ...
  - For categorical or `ARRAY<categorical>` input columns, returns `NULL`.
- `quantiles`: an `ARRAY<FLOAT64>` column that contains information about the [quantiles](https://en.wikipedia.org/wiki/Quantile) in an input column, as computed by the [`APPROX_QUANTILES`](../standard-sql/approximate_aggregate_functions.md#approx_quantiles) function. The `quantiles` column contains one of the following values:
  ...
  - For categorical or `ARRAY<categorical>` input columns, returns `NULL`.
- `unique`: an `INT64` column that contains information about the number of unique values in an input column, as computed by the [`APPROX_COUNT_DISTINCT`](../standard-sql/approximate_aggregate_functions.md#approx_count_distinct) function. The `unique` column contains one of the following values:
...
- `top_values`: a `ARRAY<STRUCT<STRING, INT64>>` column that contains information about the top values and number of occurrences in an input column, as computed by the [`APPROX_TOP_COUNT`](../standard-sql/approximate_aggregate_functions.md#approx_top_count) function. The `top_values` column contains the following fields:
...
- [Numeric](../standard-sql/data-types.md#numeric_types) types
- `STRING`
- `BOOL`
- `BYTE`
- `DATE`
