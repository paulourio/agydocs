...
- `tokenized_document`: `ARRAY<STRING>` value that represents a document that has been tokenized. A tokenized document is a collection of terms (tokens), which are used for text analysis. For more information about tokenization in BigQuery, see [`TEXT_ANALYZE`](../standard-sql/text-analysis-functions.md#text_analyze).
...
- Learn about the [`BAG_OF_WORDS`
  function](../standard-sql/text-analysis-functions.md#bag_of_words) outside of machine learning.