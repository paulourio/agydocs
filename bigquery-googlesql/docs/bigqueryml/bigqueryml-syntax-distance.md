...
> [!NOTE]
> **Note:** The [`VECTOR_SEARCH` function](../standard-sql/search_functions.md#vector_search) is another vector function that calculates the distance between vectors. You should use the `VECTOR_SEARCH` function if you need to search a dataset for vectors similar to an input vector. You should use the `ML.DISTANCE` function if you need to compare two specific vectors to determine the distance between them.
...
- `type`: a `STRING` value that specifies the type of distance to calculate.
  ...
  [`MANHATTAN`](https://en.wikipedia.org/wiki/Taxicab_geometry), and
