...
- Use the [`ML.FORECAST` function](bigqueryml-syntax-forecast.md) to retrieve the forecasted values that were generated when you created the model.
- Use the [`ML.EXPLAIN_FORECAST` function](bigqueryml-syntax-explain-forecast.md) function to retrieve the forecasted values that were generated when you created the model, and compute the prediction intervals.
- Use the [`ML.DETECT_ANOMALIES` function](bigqueryml-syntax-detect-anomalies.md) to perform [anomaly detection](https://docs.cloud.google.com/bigquery/docs/anomaly-detection-overview).

> [!NOTE]
> **Note:** If you don't want to create and manage your own model, you can use BigQuery ML's built-in [TimesFM time series model](https://docs.cloud.google.com/bigquery/docs/timesfm-model) with the [`AI.FORECAST` function](bigqueryml-syntax-ai-forecast.md) ([Preview](https://cloud.google.com/products#product-launch-stages)) to perform forecasting.
...
> [!NOTE]
> **Note:** The `ARIMA` model type is deprecated. While the model training pipelines of `ARIMA` and `ARIMA_PLUS` models are the same, `ARIMA_PLUS` supports more capabilities, including use of the [`DECOMPOSE_TIME_SERIES` option](#decompose_time_series) and the ability to work with the [`ML.ARIMA_EVALUATE`](bigqueryml-syntax-arima-evaluate.md) and [`ML.EXPLAIN_FORECAST`](bigqueryml-syntax-explain-forecast.md) functions.
...
see by using the
[`ML.EXPLAIN_FORECAST` function](bigqueryml-syntax-explain-forecast.md).
...
[`ML.EXPLAIN_FORECAST`](bigqueryml-syntax-explain-forecast.md)
...
[custom holiday modeling](#custom_holidays)
...
- `region`: Required. A `STRING` value that identifies the region to target for
  ...
  - An upper-case [holiday region code](#holiday_region). Use this option to overwrite or supplement the holidays for the specified region. You can see the holidays for a region by running `SELECT * FROM
...
- You can't use the [`TRANSFORM` clause](bigqueryml-syntax-create.md#transform) of the `CREATE MODEL` statement if you are performing custom holiday modeling.
...
[`TIME_SERIES_ID_COL`](#time_series_id_col) option to identify the dimensions
that you want to generate time series for, and the
[`HIERACHICAL_TIME_SERIES_ID_COLS`](#hierachical_time_series_id_cols) option
...
- For the input time series, the minimum length is 3 time points. The maximum length is 500,000 time points when the [`DECOMPOSE_TIME_SERIES` option](#decompose_time_series) is set `TRUE` and 1,000,000 when the option is set to `FALSE`. When forecasting multiple time series at the same time, the limit applies to each time series.
- The maximum number of time series to forecast simultaneously using the ID columns is 100,000,000.
- When forecasting multiple time series simultaneously using the ID column, any invalid time series that fail the model fitting are ignored and don't appear in the results of forecast. For example, a single point time series. A warning message is shown in this case, and you can use the [`ML.ARIMA_EVALUATE` function](bigqueryml-syntax-arima-evaluate.md) to retrieve the error message.
- The maximum time points to forecast is 10,000.
- Holiday effect modeling is effective only for approximately 5 years.
- After a multiple time series model is trained, the evaluation tab in the BigQuery page on the Google Cloud console only shows the evaluation metrics for the first 100 time series. To see the evaluation metrics for all of the time series, use the [`ML.ARIMA_EVALUATE` function](bigqueryml-syntax-arima-evaluate.md).
