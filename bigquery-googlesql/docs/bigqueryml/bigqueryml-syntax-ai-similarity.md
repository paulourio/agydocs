...
- Text data.
- Image data represented by [`ObjectRef` values](https://docs.cloud.google.com/bigquery/docs/work-with-objectref). You can create an `ObjectRef` value by passing a Cloud Storage URI to the [`OBJ.MAKE_REF` function](../standard-sql/objectref_functions.md#objmake_ref) or using an `ObjectRef` column from a table.
...
[`VECTOR_SEARCH`](../standard-sql/search_functions.md#vector_search)
...
| Embedding | Always generates embeddings for both content inputs at runtime using Agent Platform embedding LLMs. | Uses pre-computed embeddings. |
