...
[deep neural network (DNN)](https://en.wikipedia.org/wiki/Deep_learning#Deep_neural_networks)
...
You can use DNN regressor models with the
...
An `INT64` value that is less than or equal to `30` and greater than `0`. The
default value is `15`.
...
[`ML.GLOBAL_EXPLAIN` function](bigqueryml-syntax-global-explain.md)
...
[`ML.TRAINING_INFO` function](bigqueryml-syntax-train.md)
...
- `SEQ`: Split data sequentially by using the value in a specified column of one
  ...
  When you aren't running hyperparameter tuning, the last <var translate="no">n</var> rows
  ...
  When you are running hyperparameter tuning, the last <var translate="no">m</var> rows
  are used as test data, where <var translate="no">m</var> is the value specified for
  `DATA_SPLIT_TEST_FRACTION`. The second last <var translate="no">n</var> rows are used as
  evaluation data, where <var translate="no">n</var> is the value specified for
  `DATA_SPLIT_EVAL_FRACTION`. The remaining rows are used as training data.

  All rows with split values smaller than the threshold, including `NULL`s,
  are used as training data. The remaining rows are used as evaluation data.
...
[model evaluation metrics](bigqueryml-syntax-evaluate.md#output)
...
DNN classification and regression models support
...
supported for DNN classification models, see
[`DNN_CLASSIFIER`](https://docs.cloud.google.com/bigquery/docs/hp-tuning-overview#dnn_classifier).
For more information about the training objectives and hyperparameters
supported for DNN regression models, see
[`DNN_REGRESSOR`](https://docs.cloud.google.com/bigquery/docs/hp-tuning-overview#dnn_regressor).
...
- For `DNN_CLASSIFIER` models, the `label` column can contain up to 1,000 unique values; that is, the number of classes is less than or equal to 1,000. If you need to classify into more than 1,000 labels, contact [bqml-feedback@google.com](mailto:bqml-feedback@google.com).
...
Training DNN models is not supported in all BigQuery ML
