...
and
[matrix factorization models](bigqueryml-syntax-create-matrix-factorization.md).

For matrix factorization models, you can use the
[`AI.GENERATE_EMBEDDING` function](bigqueryml-syntax-ai-generate-embedding.md)
...
- `processed_input`: a `STRING` value that contains the name of the feature input column. The value of this column matches the name of the feature column provided in the [`query_statement` clause](bigqueryml-syntax-create.md#query_statement) that was used when the model was trained.
...
- `trial_id`: an `INT64` value that contains the hyperparameter tuning trial ID. This column is only returned if you ran hyperparameter tuning when creating the model.
- `processed_input`: a `STRING` value that contains the name of the user or item column. The value of this column matches the name of the user or item column provided in the [`query_statement` clause](bigqueryml-syntax-create.md#query_statement) that was used when the model was trained.
...
- `factor_weights`: an `ARRAY<STRUCT>` value that contains the factors and the weights for each factor.
  - `factor_weights.factor`: an `INT64` value that contains the latent factor from training. This value can be between `1` and the value of the [`NUM_FACTORS` option](bigqueryml-syntax-create-matrix-factorization.md#num_factors).
...
[implicit feedback](bigqueryml-syntax-create-matrix-factorization.md#feedback_type)
...
This command uses the [`UNNEST`](../standard-sql/query-syntax.md#unnest_operator)
