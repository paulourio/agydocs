...
[creating a contribution analysis model](bigqueryml-syntax-create-contribution-analysis.md)
and then calling the
[`ML.GET_INSIGHTS` function](bigqueryml-syntax-get-insights.md)
...
- `METRIC_COL`: a `STRING` expression to use to
  ...
  The values in the metric column that you use in the `METRIC_COL` option must
  be non-negative, unless you specify `0` for the `MIN_APRIORI_SUPPORT` value.
- `DIMENSION_COLS`: an `ARRAY<STRING>`
  ...
  [`ML.IMPUTER` function](bigqueryml-syntax-imputer.md),
  or by using the
  [`IFNULL` expression](../standard-sql/conditional_expressions.md#ifnull)
...
- `INTEREST_LABEL_COL`: a `STRING` value that contains the
  ...
  [choose interest and reference data](#choose-interest-reference).
...
- `apriori_support`: a numeric value that contains the apriori support value
  ...
  [apriori support threshold](bigqueryml-syntax-create-contribution-analysis.md#use_an_apriori_support_threshold)
...
- Learn how to [create a contribution analysis model](bigqueryml-syntax-create-contribution-analysis.md).