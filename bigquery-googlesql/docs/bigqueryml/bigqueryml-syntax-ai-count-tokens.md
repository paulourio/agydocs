...
- `ENDPOINT`: a `STRING` literal that specifies the name of the generative AI model to use for tokenization rules. If you don't provide an endpoint, then the default model used by the [`AI.GENERATE` function](bigqueryml-syntax-ai-generate.md) is used.
