...
> [!NOTE]
> **Note:** The [`AI.GENERATE_EMBEDDING` function](bigqueryml-syntax-ai-generate-embedding.md) offers the same functionality with simplified column names in the output. For new queries, we recommend that you use `AI.GENERATE_EMBEDDING` instead.
...
- Output data from [PCA](bigqueryml-syntax-create-pca.md), [autoencoder](bigqueryml-syntax-create-autoencoder.md), or [matrix factorization](bigqueryml-syntax-create-matrix-factorization.md) models.
...
- To generate embeddings from text or visual content,
  ...
  [remote model](bigqueryml-syntax-create-remote-model.md)
  that represents a
  [Gemini Enterprise Agent Platform embedding model](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/models#embeddings-models)
  or a
  [supported open model](bigqueryml-syntax-create-remote-model-open.md#supported_open_models),
...
- For PCA and autoencoding, `ML.GENERATE_EMBEDDING` processes the request using
  ...
  [`VECTOR_SEARCH` function](../standard-sql/search_functions.md#vector_search)
...
- For matrix factorization, `ML.GENERATE_EMBEDDING` processes the request using
  ...
  [`ML.WEIGHTS` function](bigqueryml-syntax-weights.md).
...
- `QUERY_STATEMENT`: a query whose result contains a
  ...
  [GoogleSQL query
  syntax](../standard-sql/query-syntax.md#sql_syntax).
...
- `QUERY_STATEMENT`: the GoogleSQL query
  ...
  - If you are creating embeddings from a standard table, the query must
    ...
    - For visual content embeddings, you can provide an
      [`ObjectRefRuntime`](../standard-sql/objectref_functions.md#objectrefruntime)
...
- `ml_generate_embedding_result`: this is an `ARRAY<FLOAT>` value that contains the principal components for the input data. The number of array dimensions is equal to the PCA model's [`NUM_PRINCIPAL_COMPONENTS` option](bigqueryml-syntax-create-pca.md#num_principal_components) value if that option is used when the model is created. If the [`PCA_EXPLAINED_VARIANCE_RATIO` option](bigqueryml-syntax-create-pca.md#pca_explained_variance_ratio) is used instead, the array dimensions vary depending on the input table and the option ratio determined by BigQuery ML.
...
- `ml_generate_embedding_result`: this is an `ARRAY<FLOAT>` value that contains the latent space dimensions for the input data. The number of array dimensions is equal to the number in the middle of the autoencoder model's [`HIDDEN_UNITS` option](bigqueryml-syntax-create-autoencoder.md#hidden_units) array value.
...
- `trial_id`: an `INT64` value that identifies the hyperparameter tuning trial used by the function. This column is only returned if you ran hyperparameter tuning when creating the model.
- `ml_generate_embedding_result`: this is an `ARRAY<FLOAT>` value that contains the weights of the feature, and also the intercept or bias term for the feature. The intercept value is the last value in the array. The number of array dimensions is equal to the matrix factorization model's [`NUM_FACTORS` option](bigqueryml-syntax-create-matrix-factorization.md#num_factors) value.
- `processed_input`: a `STRING` value that contains the name of the user or item column. The value of this column matches the name of the user or item column provided in the [`query_statement` clause](bigqueryml-syntax-create.md#query_statement) that was used when the matrix factorization model was trained.
...
models. For more information, see [Agent Platform and Cloud AI service
functions quotas and limits](https://docs.cloud.google.com/bigquery/quotas#cloud_ai_service_functions).

To request more quota for the Agent Platform models, use the process described in
