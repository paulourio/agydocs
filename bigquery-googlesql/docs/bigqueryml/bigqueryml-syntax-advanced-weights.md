...
- The [`CALCULATE_P_VALUES`](bigqueryml-syntax-create.md#calculate_p_values) value is `TRUE`.
- The [`CATEGORY_ENCODING_METHOD`](bigqueryml-syntax-create.md#category_encoding_method) value is `DUMMY_ENCODING`.
- The [`L1_REG`](bigqueryml-syntax-create.md#l1_reg) value is `0`.
...
- `processed_input`: a `STRING` value that contains the name of the feature column. The value of this column is the name of the feature column that's provided in the [`query_statement` clause](bigqueryml-syntax-create.md#query_statement) used during model training. If the feature is non-numeric, then there are multiple rows with the same `processed_input` value, one for each category of the feature.
...
If the [`TRANSFORM` clause](bigqueryml-syntax-create.md#transform)
