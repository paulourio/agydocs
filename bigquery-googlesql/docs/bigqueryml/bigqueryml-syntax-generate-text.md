...
> [!NOTE]
> **Note:** The [`AI.GENERATE_TEXT` function](bigqueryml-syntax-ai-generate-text.md) offers the same functionality with simplified column names in the output. For new queries, we recommend that you use `AI.GENERATE_TEXT` instead.
...
- [Remote models](bigqueryml-syntax-create-remote-model-tuned.md)
...
- `MODEL`: the name of the remote model over the
  ...
  [The `CREATE MODEL` statement for remote models over LLMs](bigqueryml-syntax-create-remote-model.md).
...
- `QUERY_STATEMENT`: the GoogleSQL query
  ...
  - Specify a `STRUCT` value that contains one or more fields. You can use the
    following types of fields within the `STRUCT` value:
    ...
    table shows some examples of `STRUCT` prompt values and how they are
    interpreted:
...
> [!NOTE]
> **Note:** We recommend against using the [`LIMIT and OFFSET` clause](../standard-sql/query-syntax.md#limit_and_offset_clause) in the prompt query. Using this clause causes the query to process all of the input data first and then apply `LIMIT` and `OFFSET`.
