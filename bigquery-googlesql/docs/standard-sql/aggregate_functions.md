...
[Aggregate function calls](aggregate-function-calls.md).

## Function list

| Name | Summary |
|---|---|
| [`AGG`](#agg) | Aggregates a measure type. |
| [`ANY_VALUE`](#any_value) | Gets an expression for some row. |
| [`APPROX_COUNT_DISTINCT`](approximate_aggregate_functions.md#approx_count_distinct) | Gets the approximate result for `COUNT(DISTINCT expression)`. For more information, see [Approximate aggregate functions](approximate_aggregate_functions.md). |
| [`APPROX_QUANTILES`](approximate_aggregate_functions.md#approx_quantiles) | Gets the approximate quantile boundaries. For more information, see [Approximate aggregate functions](approximate_aggregate_functions.md). |
| [`APPROX_TOP_COUNT`](approximate_aggregate_functions.md#approx_top_count) | Gets the approximate top elements and their approximate count. For more information, see [Approximate aggregate functions](approximate_aggregate_functions.md). |
| [`APPROX_TOP_SUM`](approximate_aggregate_functions.md#approx_top_sum) | Gets the approximate top elements and sum, based on the approximate sum of an assigned weight. For more information, see [Approximate aggregate functions](approximate_aggregate_functions.md). |
...
| [`AVG` (Differential Privacy)](aggregate-dp-functions.md#dp_avg) | `DIFFERENTIAL_PRIVACY`-supported `AVG`. Gets the differentially-private average of non-`NULL`, non-`NaN` values in a query with a `DIFFERENTIAL_PRIVACY` clause. <br /> For more information, see [Differential privacy functions](aggregate-dp-functions.md). |
...
| [`CORR`](statistical_aggregate_functions.md#corr) | Computes the Pearson coefficient of correlation of a set of number pairs. For more information, see [Statistical aggregate functions](statistical_aggregate_functions.md). |
...
| [`COUNT` (Differential Privacy)](aggregate-dp-functions.md#dp_count) | `DIFFERENTIAL_PRIVACY`-supported `COUNT`. Signature 1: Gets the differentially-private count of rows in a query with a `DIFFERENTIAL_PRIVACY` clause. <br /> Signature 2: Gets the differentially-private count of rows with a non-`NULL` expression in a query with a `DIFFERENTIAL_PRIVACY` clause. <br /> For more information, see [Differential privacy functions](aggregate-dp-functions.md). |
| [`COUNTIF`](#countif) | Gets the number of `TRUE` values for an expression. |
| [`COVAR_POP`](statistical_aggregate_functions.md#covar_pop) | Computes the population covariance of a set of number pairs. For more information, see [Statistical aggregate functions](statistical_aggregate_functions.md). |
| [`COVAR_SAMP`](statistical_aggregate_functions.md#covar_samp) | Computes the sample covariance of a set of number pairs. For more information, see [Statistical aggregate functions](statistical_aggregate_functions.md). |
...
| [`PERCENTILE_CONT` (Differential Privacy)](aggregate-dp-functions.md#dp_percentile_cont) | `DIFFERENTIAL_PRIVACY`-supported `PERCENTILE_CONT`. Computes a differentially-private percentile across privacy unit columns in a query with a `DIFFERENTIAL_PRIVACY` clause. <br /> For more information, see [Differential privacy functions](aggregate-dp-functions.md). |
...
| [`ST_CENTROID_AGG`](geography_functions.md#st_centroid_agg) | Gets the centroid of a set of `GEOGRAPHY` values. For more information, see [Geography functions](geography_functions.md). |
| [`ST_EXTENT`](geography_functions.md#st_extent) | Gets the bounding box for a group of `GEOGRAPHY` values. For more information, see [Geography functions](geography_functions.md). |
| [`ST_UNION_AGG`](geography_functions.md#st_union_agg) | Aggregates over `GEOGRAPHY` values and gets their point set union. For more information, see [Geography functions](geography_functions.md). |
| [`STDDEV`](statistical_aggregate_functions.md#stddev) | An alias of the `STDDEV_SAMP` function. For more information, see [Statistical aggregate functions](statistical_aggregate_functions.md). |
| [`STDDEV_POP`](statistical_aggregate_functions.md#stddev_pop) | Computes the population (biased) standard deviation of the values. For more information, see [Statistical aggregate functions](statistical_aggregate_functions.md). |
| [`STDDEV_SAMP`](statistical_aggregate_functions.md#stddev_samp) | Computes the sample (unbiased) standard deviation of the values. For more information, see [Statistical aggregate functions](statistical_aggregate_functions.md). |
| [`STRING_AGG`](#string_agg) | Concatenates non-`NULL` `STRING` or `BYTES` values. |
| [`SUM`](#sum) | Gets the sum of non-`NULL` values. |
| [`SUM` (Differential Privacy)](aggregate-dp-functions.md#dp_sum) | `DIFFERENTIAL_PRIVACY`-supported `SUM`. Gets the differentially-private sum of non-`NULL`, non-`NaN` values in a query with a `DIFFERENTIAL_PRIVACY` clause. <br /> For more information, see [Differential privacy functions](aggregate-dp-functions.md). |
| [`VAR_POP`](statistical_aggregate_functions.md#var_pop) | Computes the population (biased) variance of the values. For more information, see [Statistical aggregate functions](statistical_aggregate_functions.md). |
| [`VAR_SAMP`](statistical_aggregate_functions.md#var_samp) | Computes the sample (unbiased) variance of the values. For more information, see [Statistical aggregate functions](statistical_aggregate_functions.md). |
| [`VARIANCE`](statistical_aggregate_functions.md#variance) | An alias of `VAR_SAMP`. For more information, see [Statistical aggregate functions](statistical_aggregate_functions.md). |
...
Aggregates a [measure type](data-types.md#measure_type). A measure type encapsulates an
...
[multi-level aggregation](aggregate-function-calls.md#multi_level_aggregation_overcounting).
For more information and examples of using the `AGG` function
with measures, see [work with measures](https://docs.cloud.google.com/bigquery/docs/graph-measures).
...
[`GRAPH_EXPAND` TVF](../graph/graph-sql-queries.md#graph_expand), which performs a series of `LEFT JOIN`
...
If you specify a `WHERE`
clause expression, then the `IGNORE NULLS` clause is required.
...
[`AGGREGATION_THRESHOLD` clause](query-syntax.md#agg_threshold_clause).
...
[Differentially private aggregate functions](aggregate-dp-functions.md).
...
- If a [floating-point type](data-types.md#floating_point_types) is returned, the result is [non-deterministic](data-types.md#floating_point_semantics), which means you might receive a different result each time you use this function.
...
> [!NOTE]
> **Note:** If you're querying a large dataset, you can compute results faster and save resources by using [HLL++ functions](hll_functions.md) for approximate distinct counts. For more information, see [Sketches](https://docs.cloud.google.com/bigquery/docs/sketches).

**Definitions**

- `*`: Use this value to get the number of all rows in the input.
- `expression`: A value of any data type that represents the expression to evaluate. If `DISTINCT` is present, `expression` can only be a data type that is [groupable](data-types.md#groupable_data_types).
- Optional aggregate clauses: To learn more about the optional aggregate clauses that you can pass into this function, see [Aggregate function calls](aggregate-function-calls.md).

<!-- -->

- `OVER`: To learn more about the `OVER` clause and how to use it, see [Window function calls](window-function-calls.md).

**Details**

This function with `DISTINCT` supports specifying [collation](collation-concepts.md).
...
If a groupable item in the [`GROUP BY` clause](query-syntax.md#group_by_clause) is aggregated
...
You can't use the `GROUPING` function with
[multi-level aggregation](aggregate-function-calls.md#multi_level_aggregation).
...
This function supports specifying [collation](collation-concepts.md).

**Supported Argument Types**

Any [orderable data type](data-types.md#data_type_properties) except for `ARRAY`.
