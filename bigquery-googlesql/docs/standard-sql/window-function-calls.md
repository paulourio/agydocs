...
[defined in your query](query-syntax.md#window_clause), or you can
...
- [`named_window`](#ref_named_window): The name of an existing window that was defined with a [`WINDOW` clause](query-syntax.md#window_clause).
...
- `ORDER BY`: Defines how rows are ordered within a partition.
  ...
  cases for [navigation functions](navigation_functions.md).
...
[navigation functions](navigation_functions.md) and
[numbering functions](numbering_functions.md),
such as `RANK()`.
...
[`WINDOW` clause](query-syntax.md#window_clause), and referenced in
...
[`QUALIFY` clause](query-syntax.md#qualify_clause).
...
[`Produce`](#produce_table) table. The sum is computed with respect to the
...
In this example, all items in the [`Produce`](#produce_table) table are included
...
This computes a moving average in the [`Produce`](#produce_table) table.
...
count in the [`Farm`](#farm_table) table.
...
[`Produce`](#produce_table) table is referenced.
...
the [`Produce`](#produce_table) table. The window frame analyzes up to three
...
in the `OVER` clause. The [`Employees`](#employees_table) table is referenced.
