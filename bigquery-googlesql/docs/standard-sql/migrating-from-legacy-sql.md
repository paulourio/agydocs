...
[`WITH` clause](query-syntax.md#with_clause),
...
references for [arrays](data-types.md#array_type)
and [structs](data-types.md#struct_type).
...
- [GoogleSQL data types reference](data-types.md)
- [Legacy SQL data types reference](https://docs.cloud.google.com/bigquery/docs/data-types)

### `TIMESTAMP` type differences

GoogleSQL has a
[stricter range of valid `TIMESTAMP` values](data-types.md#timestamp_type)
...
[`SAFE_CAST`](conversion_functions.md#safe_casting)
...
For a more comprehensive list of reserved keywords and what constitutes valid identifiers, see the [Reserved keywords](lexical.md#reserved_keywords) section in [Lexical structure](lexical.md).
...
[Migrating table wildcard functions](#migrating_table_wildcard_functions).

### Comma operator with tables

In legacy SQL, the comma operator `,` has the non-standard meaning of
[`UNION ALL BY NAME`](query-syntax.md#set_operators) when applied to tables. In GoogleSQL, the comma operator has the
...
| Legacy SQL | GoogleSQL | Notes |
|---|---|---|
| `INTEGER(x)` | [`SAFE_CAST(x AS INT64)`](conversion_functions.md#safe_casting) |   |
| `CAST(x AS INTEGER)` | [`SAFE_CAST(x AS INT64)`](conversion_functions.md#safe_casting) |   |
| `DATEDIFF(t1, t2)` | [`DATE_DIFF(DATE(t1), DATE(t2), DAY)`](date_functions.md#date_diff) |   |
| `NOW()` | [`UNIX_MICROS(CURRENT_TIMESTAMP())`](timestamp_functions.md#current_timestamp) |   |
| `STRFTIME_UTC_USEC(ts_usec, fmt)` | [`FORMAT_TIMESTAMP(fmt, TIMESTAMP_MICROS(ts_usec))`](timestamp_functions.md#format_timestamp) |   |
| `UTC_USEC_TO_DAY(ts_usec)` | [`UNIX_MICROS(TIMESTAMP_TRUNC(TIMESTAMP_MICROS(ts_usec), DAY))`](timestamp_functions.md#timestamp_trunc) |   |
| `UTC_USEC_TO_WEEK(ts_usec, day_of_week)` | [`UNIX_MICROS(TIMESTAMP_ADD(TIMESTAMP_TRUNC(TIMESTAMP_MICROS(ts_usec), WEEK),INTERVAL day_of_week DAY))`](timestamp_functions.md#timestamp_trunc) |   |
| `REGEXP_MATCH(s, pattern)` | [`REGEXP_CONTAINS(s, pattern)`](string_functions.md#regexp_contains) |   |
| `IS_NULL(x)` | [`x IS NULL`](operators.md#is_operators) |   |
| `LEFT(s, len)` | [`SUBSTR(s, 0, len)`](string_functions.md#substr) |   |
| `RIGHT(s, len)` | [`SUBSTR(s, -len)`](string_functions.md#substr) |   |
| `s CONTAINS "foo"` | [`STRPOS(s, "foo") > 0`](string_functions.md#strpos) or [`s LIKE '%foo%'`](operators.md#comparison_operators) |   |
| `INSTR(s, "foo")` | [`STRPOS(s, "foo")`](string_functions.md#strpos) |   |
| `x % y` | [`MOD(x, y)`](mathematical_functions.md#mod) |   |
| `NEST(x)` | [`ARRAY_AGG(x)`](aggregate_functions.md#array_agg) |   |
| `GROUP_CONCAT_UNQUOTED(s, sep)` | [`STRING_AGG(s, sep)`](aggregate_functions.md#string_agg) |   |
| `SOME(x)` | `IFNULL(`[`LOGICAL_OR(x)`](operators.md#logical_operators)`, false)` |   |
| `EVERY(x)` | `IFNULL(`[`LOGICAL_AND(x)`](operators.md#logical_operators)`, true)` |   |
| `COUNT(DISTINCT x)` | [`APPROX_COUNT_DISTINCT(x)`](approximate_aggregate_functions.md#approx_count_distinct) | see [notes below](#count_function_comparison) |
| `EXACT_COUNT_DISTINCT(x)` | [`COUNT(DISTINCT x)`](aggregate_functions.md#count) | see [notes below](#count_function_comparison) |
| `QUANTILES(x, boundaries)` | [`APPROX_QUANTILES(x, boundaries-1)`](approximate_aggregate_functions.md#approx_quantiles) | output needs to be [unnested](#removing_repetition_with_flatten) |
| `TOP(x, num), COUNT(*)` | [`APPROX_TOP_COUNT(x, num)`](approximate_aggregate_functions.md#approx_top_count) |   |
| `NTH(index, arr) WITHIN RECORD` | [`arr[SAFE_ORDINAL(index)]`](operators.md#array_subscript_operator) |   |
| `COUNT(arr) WITHIN RECORD` | [`ARRAY_LENGTH(arr)`](array_functions.md#array_length) |   |
| `HOST(url)` | [`NET.HOST(url)`](net_functions.md#nethost) | see [differences below](#url_function_comparison) |
| `TLD(url)` | [`NET.PUBLIC_SUFFIX(url)`](net_functions.md#netpublic_suffix) | see [differences below](#url_function_comparison) |
| `DOMAIN(url)` | [`NET.REG_DOMAIN(url)`](net_functions.md#netreg_domain) | see [differences below](#url_function_comparison) |
| `PARSE_IP(addr_string)` | [`NET.IPV4_TO_INT64`](net_functions.md#netipv4_to_int64)`(`[`NET.IP_FROM_STRING`](net_functions.md#netip_from_string)`(addr_string))` |   |
| `FORMAT_IP(addr_int64)` | [`NET.IP_TO_STRING`](net_functions.md#netip_to_string)`(`[`NET.IPV4_FROM_INT64`](net_functions.md#netipv4_from_int64)`(addr_int64 & 0xFFFFFFFF))` |   |
| `PARSE_PACKED_IP(addr_string)` | [`NET.IP_FROM_STRING(addr_string)`](net_functions.md#netip_from_string) |   |
| `FORMAT_PACKED_IP(addr_bytes)` | [`NET.IP_TO_STRING(addr_bytes)`](net_functions.md#netip_to_string) |   |
| `NVL(expr, null_default)` | [`IFNULL(expr, null_default)`](conditional_expressions.md#ifnull) |   |

For more information about GoogleSQL functions, see
[All functions](functions-all.md).
...
[`APPROX_COUNT_DISTINCT`](approximate_aggregate_functions.md#approx_count_distinct).
...
[`NET.HOST(url)`](net_functions.md#nethost),
[`NET.PUBLIC_SUFFIX(url)`](net_functions.md#netpublic_suffix),
and [`NET.REG_DOMAIN(url)`](net_functions.md#netreg_domain).
...
[`UNNEST`](query-syntax.md#unnest_operator) topic.
...
- GoogleSQL raises an error upon division by zero, whereas legacy SQL returns `NULL`. To return `NULL` for division by zero in GoogleSQL, use [`SAFE_DIVIDE`](mathematical_functions.md#safe_divide).
- GoogleSQL raises an error for `CAST`s where the input format is invalid or out of range for the target type, whereas legacy SQL returns `NULL`. To avoid raising an error for an invalid cast in GoogleSQL, use [`SAFE_CAST`](conversion_functions.md#safe_casting).
...
[comparison operators](operators.md#comparison_operators)
...
And using [BigQuery Scripting (procedural SQL)](procedural-language.md):
...
[`INFORMATION_SCHEMA.PARTITIONS`](../information-schema/information-schema-partitions.md)
view instead.

> [!NOTE]
> **Note:** `INFORMATION_SCHEMA.PARTITIONS` is in [Preview](https://cloud.google.com/products/#product-launch-stages). For more information, see the [documentation](../information-schema/information-schema-partitions.md).
...
> [!WARNING]
> **Warning:** Partition-level `creation_time` isn't available in `INFORMATION_SCHEMA.PARTITIONS` and is set to `NULL` in the example. For table-level creation time, use the [`INFORMATION_SCHEMA.TABLES` `creation_time`](../information-schema/information-schema-tables.md#schema) field.
...
- GoogleSQL: The `INFORMATION_SCHEMA.PARTITIONS` view requires the [`bigquery.tables.get` and `bigquery.tables.list`](../information-schema/information-schema-partitions.md#required_permissions) permissions. This change in permissions for GoogleSQL lets you grant metadata-only roles (for example, [`roles/bigquery.metadataViewer`](https://docs.cloud.google.com/iam/docs/roles-permissions/bigquery#bigquery.metadataViewer)) without providing access to the underlying table data.