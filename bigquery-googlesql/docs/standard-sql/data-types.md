...
[Lexical Structure and Syntax](lexical.md#literals).
...
the [`ORDER BY` clause](query-syntax.md#order_by_clause).
...
- To compare geography values, use [ST_Equals](geography_functions.md#st_equals).
...
- All types that support comparisons can be used in a `JOIN` condition. See [JOIN Types](query-syntax.md#join_types) for an explanation of join conditions.
...
| `GEOGRAPHY` | 16 logical bytes + 24 logical bytes \* the number of vertices in the geography type. To verify the number of vertices, use the [`ST_NumPoints`](geography_functions.md#st_numpoints) function. |
...
see [Array literals](lexical.md#array_literals).
...
[`GENERATE_ARRAY`](array_functions.md#generate_array)
...
[`GENERATE_DATE_ARRAY`](array_functions.md#generate_date_array)
...
using the
[`FROM_BASE64` function](string_functions.md#from_base64).
You can also convert a sequence of `BYTES` into a base64-encoded `STRING`
expression using the
[`TO_BASE64` function](string_functions.md#to_base64).

To learn more about the literal representation of a bytes type,
see [Bytes literals](lexical.md#string_and_bytes_literals).
...
see [Date literals](lexical.md#date_literals).
...
see [Datetime literals](lexical.md#datetime_literals).
...
[Geography Function](geography_functions.md).
...
[`FinGraph`](../graph/graph-schema-statements.md#fin_graph) property graph:

    GRAPH graph_db.FinGraph
    MATCH (n:Person)
    RETURN n.name

In the following example, the [`TYPEOF`](utility-functions.md#typeof) function is used to inspect the
...
see [Interval literals](lexical.md#interval_literals).

### Constructing an interval

You can construct an interval with an interval literal that supports
a [single datetime part](#single_datetime_part_interval) or a
[datetime part range](#range_datetime_part_interval).
...
[interval-supported datetime part](#interval_datetime_parts). For example:
...
For additional examples, see [Interval literals](lexical.md#interval_literal_single).
...
[interval-supported datetime parts](#interval_datetime_parts):
...
For additional examples, see [Interval literals](lexical.md#interval_literal_range).
...
see [JSON literals](lexical.md#json_literals).
...
The measure type is used by only the [`AGG` function](aggregate_functions.md#agg). Measures
...
[element properties definition](../graph/graph-schema-statements.md#element_table_property_definition).
...
- Supported by only the `AGG` function on output from the [`GRAPH_EXPAND` TVF](../graph/graph-sql-queries.md#graph_expand).
...
see [Integer literals](lexical.md#integer_literals).
...
see [`NUMERIC` literals](lexical.md#numeric_literals).

To learn more about the literal representation of a `BIGNUMERIC` type,
see [`BIGNUMERIC` literals](lexical.md#bignumeric_literals).
...
see [Floating point literals](lexical.md#floating_point_literals).
...
can be compared,
see [Ordering floating point values](#orderable_floating_points).
...
You can construct a range with the [`RANGE` constructor](#range_with_constructor)
or a [range literal](#range_with_literal).

#### Construct a range with a constructor

You can construct a range with the `RANGE` constructor. To learn more,
see [`RANGE`](range-functions.md#range).
...
- `lower_bound`: The range starts from this value. This can be a [date](lexical.md#date_literals), [datetime](lexical.md#datetime_literals), or [timestamp](lexical.md#timestamp_literals) literal. If this value is `UNBOUNDED` or `NULL`, the range doesn't include a lower bound.
- `upper_bound`: The range ends before this value. This can be a [date](lexical.md#date_literals), [datetime](lexical.md#datetime_literals), or [timestamp](lexical.md#timestamp_literals) literal. If this value is `UNBOUNDED` or `NULL`, the range doesn't include an upper bound.
...
see [Range literals](lexical.md#range_literals).
...
see [String literals](lexical.md#string_and_bytes_literals).
...
see [Struct literals](lexical.md#struct_literals).
...
To represent
an absolute point in time, use a [timestamp](#timestamp_type).
...
see [Time literals](lexical.md#time_literals).
...
see [Timestamp literals](lexical.md#timestamp_literals).
