...
| [`ARRAY_AGG`](aggregate_functions.md#array_agg) | Gets an array of values. For more information, see [Aggregate functions](aggregate_functions.md). |
| [`ARRAY_CONCAT`](#array_concat) | Concatenates one or more arrays with the same element type into a single array. |
| [`ARRAY_CONCAT_AGG`](aggregate_functions.md#array_concat_agg) | Concatenates arrays and returns a single array as a result. For more information, see [Aggregate functions](aggregate_functions.md). |
| [`ARRAY_FIRST`](#array_first) | Gets the first element in an array. |
| [`ARRAY_LAST`](#array_last) | Gets the last element in an array. |
...
| [`ARRAY_SLICE`](#array_slice) | Produces an array containing zero or more consecutive elements from an input array. |
...
| [`GENERATE_RANGE_ARRAY`](range-functions.md#generate_range_array) | Splits a range into an array of subranges. For more information, see [Range functions](range-functions.md). |
| [`GENERATE_TIMESTAMP_ARRAY`](#generate_timestamp_array) | Generates an array of timestamps in a range. |
| [`JSON_ARRAY`](json_functions.md#json_array) | Creates a JSON array. For more information, see [JSON functions](json_functions.md). |
| [`JSON_ARRAY_APPEND`](json_functions.md#json_array_append) | Appends JSON data to the end of a JSON array. For more information, see [JSON functions](json_functions.md). |
| [`JSON_ARRAY_INSERT`](json_functions.md#json_array_insert) | Inserts JSON data into a JSON array. For more information, see [JSON functions](json_functions.md). |
| [`JSON_EXTRACT_ARRAY`](json_functions.md#json_extract_array) | (Deprecated) Extracts a JSON array and converts it to a SQL `ARRAY<JSON-formatted STRING>` or `ARRAY<JSON>` value. For more information, see [JSON functions](json_functions.md). |
| [`JSON_EXTRACT_STRING_ARRAY`](json_functions.md#json_extract_string_array) | (Deprecated) Extracts a JSON array of scalar values and converts it to a SQL `ARRAY<STRING>` value. For more information, see [JSON functions](json_functions.md). |
| [`JSON_QUERY_ARRAY`](json_functions.md#json_query_array) | Extracts a JSON array and converts it to a SQL `ARRAY<JSON-formatted STRING>` or `ARRAY<JSON>` value. For more information, see [JSON functions](json_functions.md). |
| [`JSON_VALUE_ARRAY`](json_functions.md#json_value_array) | Extracts a JSON array of scalar values and converts it to a SQL `ARRAY<STRING>` value. For more information, see [JSON functions](json_functions.md). |
| [`RANGE_BUCKET`](mathematical_functions.md#range_bucket) | Scans through a sorted array and returns the 0-based position of a point's upper bound. For more information, see [Mathematical functions](mathematical_functions.md). |
...
- If the subquery returns an `ARRAY` typed column or `ARRAY` typed rows, the `ARRAY` function returns an error that GoogleSQL doesn't support `ARRAY`s with elements of type [`ARRAY`](data-types.md#array_type).
...
> [!NOTE]
> **Note:** You can also use the [\|\| concatenation operator](operators.md) to concatenate arrays.
...
[Array subscript operator](operators.md#array_subscript_operator) and [Accessing array
