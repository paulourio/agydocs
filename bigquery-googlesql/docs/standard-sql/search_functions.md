...
search terms appear in the data, based on the [rules for search_query](#search_query_rules)
and text analysis described in the [text analyzer](text-analysis.md). Otherwise,
...
- `analyzer`: A named argument with a `STRING` value. Takes
  ...
  - `'LOG_ANALYZER'` (default): Breaks the input into tokens when delimiters
    ...
    For more information, see [`LOG_ANALYZER`](text-analysis.md#log_analyzer).
  ...
  - `'NO_OP_ANALYZER'`: Extracts the text as a single token, but
    ...
    see [`NO_OP_ANALYZER`](text-analysis.md#no_op_analyzer).

  - `'PATTERN_ANALYZER'`: Breaks the input into tokens that match a
    regular expression. For more information, see
    [`PATTERN_ANALYZER` text analyzer](text-analysis.md#pattern_analyzer).
...
- `analyzer_options`: A named argument with a JSON-formatted `STRING` value.
  ...
  see [Text analyzer options](text-analysis.md#text_analyzer_options).
...
- Text not enclosed in backticks requires the following
  ...
  - If the quoted string is preceded by the character `r` or `R`, such as
    ...
    sequences, see [String and byte literals](lexical.md#literals).
...
whole tables, using the default [`LOG_ANALYZER`](text-analysis.md#log_analyzer) text analyzer
...
default [`LOG_ANALYZER`](text-analysis.md#log_analyzer) text analyzer with
...
The following query shows additional ways to search, using the
default [`LOG_ANALYZER`](text-analysis.md#log_analyzer) text analyzer with custom
...
[`NO_OP_ANALYZER`](text-analysis.md#no_op_analyzer) text analyzer:
...
The following query shows how to use the [`PATTERN_ANALYZER`](text-analysis.md#pattern_analyzer)
...
The following query shows additional ways to search, using the
[`PATTERN_ANALYZER`](text-analysis.md#pattern_analyzer) text analyzer with
...
see the [Text analysis](text-analysis.md) reference guide.
...
You can also use the `VECTOR_SEARCH` function
with `ObjectRef` input. For more information, see
[Search with `ObjectRef` input](#objectref-search).

**Definitions**

- `base_table`: The table to search for nearest neighbor embeddings.
- `base_table_query`: A query that you can use to pre-filter the base table. Only `SELECT`, `FROM`, and `WHERE` clauses are allowed in this query. Don't apply any filters to the embedding column. You can't use [logical views](https://docs.cloud.google.com/bigquery/docs/views-intro) in this query. Using a [subquery](subqueries.md) might interfere with index usage or cause your query to fail. If the base table is indexed and the `WHERE` clause contains columns that are not stored in the index, then `VECTOR_SEARCH` post-filters on those columns instead. To learn more, see [Store columns and pre-filter](https://docs.cloud.google.com/bigquery/docs/vector-index#stored-columns).
...
- `query_table`: The table that provides the
  embeddings, strings, or `ObjectRef`s for which to find nearest neighbors.
  All columns are passed through as output columns.
...
- `distance_type`: A named argument with a `STRING` value.
  ...
  type specified in the [`distance_type` option](data-definition-language.md#vector_index_option_list) of the
...
- `options`: A named argument with a JSON-formatted `STRING` value.
  ...
  - `fraction_lists_to_search`: A JSON number that specifies the
    ...
    [`num_lists` option](data-definition-language.md#vector_index_option_list) in the `ivf_options` option or derived from
    the [`leaf_node_embedding_count` option](data-definition-language.md#vector_index_option_list) in the
...
[`AI.SEARCH` function](../bigqueryml/bigqueryml-syntax-ai-search.md)
...
In addition to performing `VECTOR_SEARCH` using `STRING` input,
you can also use `ObjectRef` input.

The following example shows how to use the `VECTOR_SEARCH` function to perform
semantic search on `ObjectRef`s:
...
- If the base table has [autonomous embedding generation](https://docs.cloud.google.com/bigquery/docs/autonomous-embedding-generation)
  enabled and your `query_column_to_search` column is a `STRING` or `ObjectRef`
  column, then the following limitations apply:
