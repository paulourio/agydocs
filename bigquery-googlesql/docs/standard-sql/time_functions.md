...
| [`TIME_DIFF`](#time_diff) | Gets the number of unit boundaries between two `TIME` values at a particular time granularity. |
| [`TIME_SUB`](#time_sub) | Subtracts a specified time interval from a `TIME` value. |
| [`TIME_TRUNC`](#time_trunc) | Truncates a `TIME` value at a particular granularity. |
...
See [Time zone definitions](timestamp_functions.md#timezone_definitions) for information
...
- `format_string`: A `STRING` value that contains the [format elements](format-elements.md#format_elements_date_time) to use with `time_expr`.
...
- `format_string`: A `STRING` value that contains the [format elements](format-elements.md#format_elements_date_time) to use with `time_string`.
...
2. Constructs a `TIME` object using a `TIMESTAMP` object. It supports an optional parameter to [specify a time zone](timestamp_functions.md#timezone_definitions). If no time zone is specified, the default time zone, UTC, is used.
