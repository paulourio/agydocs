...
A subquery is a [query](query-syntax.md) that appears inside another
...
players, using the [`Players`](#example_tables) and [`Mascots`](#example_tables)
tables:
...
When the subquery is written with`SELECT AS STRUCT`,
...
See [Array functions](array_functions.md#array) for full semantics.

**Examples**

In this example, an ARRAY subquery returns an array of usernames assigned to the
red team in the [`NPCs`](#example_tables) table:
...
`corba` exists within the [`Players`](#example_tables) table:
...
produced, using the [`Players`](#example_tables) table:
...
In this example, a list of [`NPCs`](#example_tables)
...
more about this [here](#evaluation_rules_subqueries).

**Examples**

In this example, a list of mascots that don't have any players assigned to them
are returned. The [`Mascots`](#example_tables) and [`Players`](#example_tables)
