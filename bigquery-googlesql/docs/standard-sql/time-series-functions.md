...
| Name | Summary |
|---|---|
| [`APPENDS`](#appends) | Returns all rows appended to a table for a given time range. |
| [`CHANGES`](#changes) | Returns all rows that have changed in a table for a given time range. |
...
- [`CREATE TABLE` DDL statement](data-definition-language.md#create_table_statement)
- [`INSERT` DML statement](dml-syntax.md#insert_statement)
- [Data appended as part of a `MERGE` DML statement](dml-syntax.md#merge_statement)
...
- `start_timestamp`: a [`TIMESTAMP`](data-types.md#timestamp_type)
  ...
  **Note:** An error is returned if the `start_timestamp` is older than
  allowed by [time travel](https://docs.cloud.google.com/bigquery/docs/time-travel). An error is also returned if the
  `start_timestamp` value is `NULL` and the table was created earlier than
  allowed by time travel. To get all available appended records,
  specify a `start_timestamp` within your time travel window instead of
  `NULL`. For standard tables, the time travel window is seven days, but you can
  [configure the time travel window](https://docs.cloud.google.com/bigquery/docs/time-travel#configure_the_time_travel_window) to be less than that.
...
[`enable_change_history` option](data-definition-language.md#table_option_list)
to `TRUE`.
...
- [`CREATE TABLE` DDL statement](data-definition-language.md#create_table_statement)
- [`INSERT` DML statement](dml-syntax.md#insert_statement)
- [Data appended, changed or deleted as part of a `MERGE` DML statement](dml-syntax.md#merge_statement)
- [`UPDATE` DML statement](dml-syntax.md#update_statement)
- [`DELETE` DML statement](dml-syntax.md#delete_statement)
- [Loading data](https://docs.cloud.google.com/bigquery/docs/loading-data) into BigQuery
- [Streaming ingestion](https://docs.cloud.google.com/bigquery/docs/write-api#use_data_manipulation_language_dml_with_recently_streamed_data)
- [`TRUNCATE TABLE` DML statement](dml-syntax.md#truncate_table_statement)
...
- `table`: the BigQuery table name. This must be a regular BigQuery table, and must have the [`enable_change_history`
  option](data-definition-language.md#table_option_list) set to `TRUE`. Enabling this table option has an impact on costs; for more information see [Pricing and costs](https://docs.cloud.google.com/bigquery/docs/change-history#pricing_and_costs). This argument must be preceded by the word `TABLE`.
- `start_timestamp`: a [`TIMESTAMP`](data-types.md#timestamp_type) value indicating the earliest
  ...
  **Note:** An error is returned if the `start_timestamp` is older than
  allowed by [time travel](https://docs.cloud.google.com/bigquery/docs/time-travel). An error is also returned if the
  `start_timestamp` value is `NULL` and the table was created earlier than
  allowed by time travel. To get all available changes,
  specify a `start_timestamp` within your time travel window instead of
  `NULL`. For standard tables, the time travel window is seven days, but you
  can [configure the time travel window](https://docs.cloud.google.com/bigquery/docs/time-travel#configure_the_time_travel_window) to be less than that.
...
[`ALTER TABLE SET OPTIONS` DDL statement](data-definition-language.md#alter_table_set_options_statement).
...
- `bucket_width`: An `INTERVAL` value that represents the width of a date bucket. A [single interval](data-types.md#single_datetime_part_interval) with [date parts](data-types.md#interval_datetime_parts) is supported.
...
- `bucket_width`: An `INTERVAL` value that represents the width of a datetime bucket. A [single interval](data-types.md#single_datetime_part_interval) with [date and time parts](data-types.md#interval_datetime_parts) is supported.
...
- `bucket_width`: An `INTERVAL` value that represents the width of a timestamp bucket. A [single interval](data-types.md#single_datetime_part_interval) with [date and time parts](data-types.md#interval_datetime_parts) is supported.
