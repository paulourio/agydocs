...
> [!NOTE]
> **Note:** `SELECT * EXCEPT` doesn't exclude columns that don't have names. Also, `SELECT * EXCEPT` and the [`EXCEPT`](#except) set operator have different use cases and rules.
...
and array subqueries (see [Subqueries](subqueries.md)) are normally not
...
See [MATCH_RECOGNIZE clause](#match_recognize_clause).

#### `graph_table_operator`

See [GRAPH_TABLE operator](../graph/graph-sql-queries.md#graph_table_operator).
...
`( query_expr ) [ [ AS ] alias ]` is a [table subquery](subqueries.md#table_subquery_concepts).
...
Common table expressions (CTEs) in a [`WITH` Clause](#with_clause) act like
...
element in the array. The output of `UNNEST` is one [value table](#value_tables) column.
...
- `array_path`: The
  ...
  The `UNNEST` operation with any [correlated](#correlated_join) `array_path` must
...
[`IN` operator](operators.md#in_operators).
...
see [Explicit and implicit `UNNEST`](#explicit_implicit_unnest).
...
- `aggregate_function_call`: An aggregate function call that aggregates all input rows where `input_column` matches each `value`, [following these
  rules](#rules_for_pivot_agg_function). Each aggregation corresponding to a different `value` produces a different column in the output.
...
- You can't use [multi-level aggregation](aggregate-function-calls.md#multi_level_aggregation) within a `PIVOT` operator call.
...
follows the same rules as the standard [`ORDER BY` clause](#order_by_clause).
The data type of each
expression must be [orderable](data-types.md#orderable_data_types).
...
| `<symbol>` | Matches a single row such that the associated expression in the `DEFINE` clause evaluates to `TRUE` for that row. See the [`DEFINE` clause](#match_recognize_define) for details. |
...
[match disambiguation](#match_disambiguation).
...
[`GRAPH_TABLE` operator](../graph/graph-sql-queries.md#graph_table_operator) in the
...
This query performs an `INNER JOIN` on the [`Roster`](#roster_table)
and [`TeamMascot`](#teammascot_table) tables.
...
You can use a [correlated](#correlated_join) `INNER JOIN` to flatten an array
...
You can use a [correlated](#correlated_join) cross join to convert or
...
This query performs an `CROSS JOIN` on the [`Roster`](#roster_table)
and [`TeamMascot`](#teammascot_table) tables.
...
[`CROSS JOIN`](#cross_join)s can be written implicitly with a comma. This is
...
[Join operations in a sequence](#sequences_of_joins).

    FROM (A, B)  // INVALID

You can use a [correlated](#correlated_join) comma cross join to convert or
flatten an array into a set of rows. To learn more, see
[Convert elements in an array to rows in a table](https://docs.cloud.google.com/bigquery/docs/arrays#flattening_arrays).

**Examples**

This query performs a comma cross join on the [`Roster`](#roster_table)
and [`TeamMascot`](#teammascot_table) tables.
...
This query performs a `FULL JOIN` on the [`Roster`](#roster_table)
and [`TeamMascot`](#teammascot_table) tables.
...
This query performs a `LEFT JOIN` on the [`Roster`](#roster_table)
and [`TeamMascot`](#teammascot_table) tables.
...
This query performs a `RIGHT JOIN` on the [`Roster`](#roster_table)
and [`TeamMascot`](#teammascot_table) tables.
...
A [join operation](#join_types) pairs rows from two `from_item`s, and a join
condition filters those pairs to determine which rows appear in the output.

The two types of join conditions are the [`ON` clause](#on_clause) and
[`USING` clause](#using_clause).

When you use the `JOIN` keyword, you typically need an
`ON` or `USING` clause to specify the join
condition. An
exception is the [`CROSS JOIN` operator](#cross_join), which doesn't support an
`ON` or `USING` clause.
...
- `bool_expression`: The boolean expression that specifies the condition for the join. This is frequently a [comparison operation](operators.md#comparison_operators) or logical combination of comparison operators.
...
[equality comparison operation](operators.md#comparison_operators) on the columns named in
...
are populated by [coalescing](conditional_expressions.md#coalesce) the values from the left and right
...
[`Roster`](#roster_table) and [`TeamMascot`](#teammascot_table) table.
...
The [`ON`](#on_clause) and [`USING`](#using_clause) join conditions aren't
...
a [correlated subquery](subqueries.md#correlated_subquery_concepts):
...
The [`UNNEST` operator](#unnest_operator) can be explicit or implicit.
...
[Roster](#roster_table) and [PlayerStats](#playerstats_table) tables:
...
This query returns returns all rows from the [`Roster`](#roster_table) table
...
The `GROUP BY` clause can include [groupable](data-types.md#data_type_properties) expressions
and their ordinals.
...
`GROUP BY` clause, see [Groupable data types](data-types.md#data_type_properties).
...
- Expressions that include [aggregate functions](aggregate-function-calls.md).
- Expressions that include [window functions](window-function-calls.md).
...
- `groupable_item`: Group rows in a table that share common values for certain columns. To learn more, see [Group rows by groupable items](#group_by_grouping_item). [Anonymous `STRUCT` values](data-types.md#tuple_syntax) aren't allowed.
...
[`GROUPING` function](aggregate_functions.md#grouping)
...
You can include [`ROLLUP`](#group_by_rollup) in a
...
You can include [`CUBE`](#group_by_cube) in a `GROUP BY GROUPING SETS` clause.
For example:
...
[recursive](#recursive_cte) or [non-recursive](#simple_cte) common table
...
[`Produce`](window-function-calls.md#produce_table) table and their rank.
...
[`Produce`](window-function-calls.md#produce_table) table.
...
[window function](window-function-calls.md). A named window can be defined with
a [window specification](window-function-calls.md#def_window_spec) or reference another
...
These examples reference a table called [`Produce`](window-function-calls.md#produce_table).
They all return the same [result](window-function-calls.md#def_use_named_window). Note the different
...
use the [`BY NAME` or `CORRESPONDING`](#by_name_or_corresponding) modifier.
...
query expression. Each CTE binds the results of a [subquery](subqueries.md)
to a table name, which can be used elsewhere in the same query expression,
but [rules apply](#cte_rules).

CTEs can be [non-recursive](#simple_cte) or
[recursive](#recursive_cte) and you can include both of these in your
...
a non-recursive [subquery](subqueries.md)
...
recursive [subquery](subqueries.md) and a name associated with the CTE.
...
inside an [expression subquery](subqueries.md#expression_subquery_concepts)
...
- `CREATE RECURSIVE VIEW` isn't supported. To work around this, use the `WITH RECURSIVE` clause as the `query_expression` in the `CREATE VIEW` statement. For more information, see [CREATE VIEW](data-definition-language.md#create_view_statement).
...
You can't use the `AGGREGATION_THRESHOLD` clause with a
[multi-level aggregation](aggregate-function-calls.md#multi_level_aggregation) query.
...
[differentially private aggregations](aggregate-dp-functions.md). To learn more about
differential privacy, see [Differential privacy](https://docs.cloud.google.com/bigquery/docs/differential-privacy).

You can't use differential privacy (or the deprecated `ANONYMIZATION` clause
it replaces) with a [multi-level aggregation](aggregate-function-calls.md#multi_level_aggregation) query.
...
see [Privacy parameters](#dp_privacy_parameters).
...
[joins](#join_types), and [parenthesized joins](#join_types).
...
Value tables can also occur as the output of the [`UNNEST`](#unnest_operator)
operator or a [subquery](subqueries.md). The [`WITH` clause](#with_clause)
...
array [subqueries](subqueries.md) normally require a single-column query,
...
[`Roster`](#roster_table) and [`TeamMascot`](#teammascot_table),
and [`PlayerStats`](#playerstats_table) tables.
