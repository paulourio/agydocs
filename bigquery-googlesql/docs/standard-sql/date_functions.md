...
| [`DATE_DIFF`](#date_diff) | Gets the number of unit boundaries between two `DATE` values at a particular time granularity. |
...
| [`DATE_TRUNC`](#date_trunc) | Truncates a `DATE`, `DATETIME`, or `TIMESTAMP` value at a particular granularity. |
...
| [`GENERATE_DATE_ARRAY`](array_functions.md#generate_date_array) | Generates an array of dates in a range. For more information, see [Array functions](array_functions.md). |
...
- `time_zone_expression`: A `STRING` expression that represents a [time zone](timestamp_functions.md#timezone_definitions). If no time zone is specified, the default time zone, UTC, is used. If this expression is used and it evaluates to `NULL`, this function returns `NULL`.
...
- `time_zone_expression`: A `STRING` expression that represents a [time zone](timestamp_functions.md#timezone_definitions). If no time zone is specified with `timestamp_expression`, the default time zone, UTC, is used.
...
- `format_string`: A `STRING` value that contains the [format elements](format-elements.md#format_elements_date_time) to use with `date_expr`.
...
- `format_string`: A `STRING` value that contains the [format elements](format-elements.md#format_elements_date_time) to use with `date_string`.
