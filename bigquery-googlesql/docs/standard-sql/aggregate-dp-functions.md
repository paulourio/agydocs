...
[differential privacy clause](query-syntax.md#dp_clause).
...
| Name | Summary |
|---|---|
| [`AVG` (Differential Privacy)](#dp_avg) | `DIFFERENTIAL_PRIVACY`-supported `AVG`. Gets the differentially-private average of non-`NULL`, non-`NaN` values in a query with a `DIFFERENTIAL_PRIVACY` clause. |
| [`COUNT` (Differential Privacy)](#dp_count) | `DIFFERENTIAL_PRIVACY`-supported `COUNT`. Signature 1: Gets the differentially-private count of rows in a query with a `DIFFERENTIAL_PRIVACY` clause. <br /> Signature 2: Gets the differentially-private count of rows with a non-`NULL` expression in a query with a `DIFFERENTIAL_PRIVACY` clause. |
| [`PERCENTILE_CONT` (Differential Privacy)](#dp_percentile_cont) | `DIFFERENTIAL_PRIVACY`-supported `PERCENTILE_CONT`. Computes a differentially-private percentile across privacy unit columns in a query with a `DIFFERENTIAL_PRIVACY` clause. |
| [`SUM` (Differential Privacy)](#dp_sum) | `DIFFERENTIAL_PRIVACY`-supported `SUM`. Gets the differentially-private sum of non-`NULL`, non-`NaN` values in a query with a `DIFFERENTIAL_PRIVACY` clause. |
...
This function must be used with the [`DIFFERENTIAL_PRIVACY` clause](query-syntax.md#dp_clause)
...
references a table called [`professors`](query-syntax.md#dp_example_tables).
...
> [!NOTE]
> **Note:** For more information about when and when not to use noise, see [Remove noise](query-syntax.md#eliminate_noise).
...
[differentially private](query-syntax.md#dp_clause) `FROM` clause. The final result
...
[`professors`](query-syntax.md#dp_example_tables).
...
view called [`professors`](query-syntax.md#dp_example_tables).
...
One of the following [supertypes](conversion_rules.md#supertypes):
...
> [!NOTE]
> **Note:** For more information about when and when not to use noise, see [Use differential privacy](query-syntax.md#eliminate_noise).

## Supplemental materials

### Clamp values in a differentially private aggregate function

In [differentially private queries](query-syntax.md#dp_clause),
