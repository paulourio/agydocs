...
- `GROUPING`
- Aggregate functions with [multi-level aggregation](aggregate-function-calls.md#multi_level_aggregation)
...
- [Operators](operators.md), such as `+` and `=`, don't support the `SAFE.` prefix. To prevent errors from a division operation, use [SAFE_DIVIDE](mathematical_functions.md#safe_divide).
- Some operators, such as `IN`, `ARRAY`, and `UNNEST`, resemble functions but don't support the `SAFE.` prefix.
- The `CAST` and `EXTRACT` functions don't support the `SAFE.` prefix. To prevent errors from casting, use [SAFE_CAST](conversion_functions.md#safe_casting).
...
[math functions](mathematical_functions.md), [DATE functions](date_functions.md),
[DATETIME functions](datetime_functions.md),
[TIMESTAMP functions](timestamp_functions.md), and [JSON functions](json_functions.md).
BigQuery does not support the use of the `SAFE.` prefix with
[aggregate](aggregate_functions.md), [window](window-function-calls.md), or
