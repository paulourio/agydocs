...
GoogleSQL also supports [system variables](../system/system-variables.md). You don't
...
[`SELECT AS STRUCT` query](query-syntax.md#select_as_struct)
...
- `sql_expression`: An expression that can represent one of the following:
  ...
  - A [query statement](query-syntax.md)
  - An expression that you can use on a query statement
  - A single [DDL statement](data-definition-language.md)
  - A single [DML statement](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-manipulation-language)

  - A single [DCL statement](data-control-language.md)

  This expression can't be a control statement like `IF`.
- `expression`: Can be a
  [function](functions-all.md), [conditional expression](conditional_expressions.md), or
  [expression subquery](subqueries.md#expression_subquery_concepts).
...
The transaction ends when a [`COMMIT TRANSACTION`](#commit_transaction) or
[`ROLLBACK TRANSACTION`](#rollback_transaction) statement is reached. If
...
proper GoogleSQL [type](data-types.md). The same variable may not appear
