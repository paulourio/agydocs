...
| [`DATETIME_DIFF`](#datetime_diff) | Gets the number of unit boundaries between two `DATETIME` values at a particular time granularity. |
| [`DATETIME_SUB`](#datetime_sub) | Subtracts a specified time interval from a `DATETIME` value. |
| [`DATETIME_TRUNC`](#datetime_trunc) | Truncates a `DATETIME` or `TIMESTAMP` value at a particular granularity. |
...
See [Time zone definitions](timestamp_functions.md#timezone_definitions) for
...
3. Constructs a `DATETIME` object using a `TIMESTAMP` object. It supports an optional parameter to [specify a time zone](timestamp_functions.md#timezone_definitions). If no time zone is specified, the default time zone, UTC, is used.
...
- `format_string`: A `STRING` value that contains the [format elements](format-elements.md#format_elements_date_time) to use with `datetime_expr`.
...
- `format_string`: A `STRING` value that contains the [format elements](format-elements.md#format_elements_date_time) to use with `datetime_string`.
