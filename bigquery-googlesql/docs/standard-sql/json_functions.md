...
| [`JSON_FLATTEN`](#json_flatten) | Produces a new SQL `ARRAY<JSON>` value containing all non-array values that are either directly in the input JSON value or children of one or more consecutively nested arrays in the input JSON value. |
| [`JSON_KEYS`](#json_keys) | Extracts unique JSON keys from a JSON expression. |
...
| [`STRING` (JSON)](#string_for_json) | Converts a JSON string to a SQL `STRING` value. |
...
- `json_path_value_pair`: A value and the [JSONPath](#JSONPath_format) for
  ...
  - `json_path`: Insert `value` at this [JSONPath](#JSONPath_format)
    in `json_expr`.
...
> [!NOTE]
> **Note:** This function is deprecated. Consider using [JSON_QUERY](#json_query).
...
[JSONPath](#JSONPath_format) characters in JSON keys. For example: `['a.b']`.
...
> [!NOTE]
> **Note:** This function is deprecated. Consider using [JSON_QUERY_ARRAY](#json_query_array).
...
> [!NOTE]
> **Note:** This function is deprecated. Consider using [JSON_VALUE](#json_value).
...
> [!NOTE]
> **Note:** This function is deprecated. Consider using [JSON_VALUE_ARRAY](#json_value_array).
...
- `json_path_value_pair`: A value and the [JSONPath](#JSONPath_format) for
  ...
  - `json_path`: Insert or replace `value` at this [JSONPath](#JSONPath_format)
    in `json_expr`.
...
- `json_path`: Remove JSON nulls at this [JSONPath](#JSONPath_format) for
  `json_expr`.
...
- [`JSON`](data-types.md#json_type) type
- `STRING` type
