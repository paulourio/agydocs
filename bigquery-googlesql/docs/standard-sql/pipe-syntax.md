...
concise than [standard query syntax](query-syntax.md). Pipe syntax supports the
...
- [`SELECT`](#select_pipe_operator) (except when using `SELECT DISTINCT` or window functions)
...
In pipe syntax, a query can start with a standard [`FROM` clause](query-syntax.md#from_clause)
and use any standard `FROM` syntax, including tables, joins, subqueries,
and
table-valued functions (TVFs). Table aliases can be
assigned to each input item using the [`AS alias` clause](query-syntax.md#using_aliases).
...
[`SELECT * FROM table_name`](query-syntax.md#select_star) in standard syntax.
...
[`SELECT` clause](query-syntax.md#select_list) in a table subquery in standard syntax. The
`SELECT` operator supports standard output modifiers like
`SELECT AS STRUCT` and `SELECT DISTINCT`.
The `SELECT` operator also supports [window functions](window-function-calls.md),
including [named windows](window-function-calls.md#def_use_named_window). Named windows are defined using the
...
[`SELECT *` statement](query-syntax.md#select_star) in standard syntax produces.
...
[`SELECT *, new_column`](query-syntax.md#select_star) in standard syntax. The `EXTEND` operator supports
...
[`SELECT * REPLACE (expression AS column)`](query-syntax.md#select_replace) in standard syntax.
...
[`SELECT * EXCEPT (column)`](query-syntax.md#select_except) in standard syntax. Each
...
[`AS alias` clause](query-syntax.md#using_aliases) on a table subquery in standard syntax. Any
...
as the [`WHERE` clause](query-syntax.md#where_clause) in standard syntax.

In pipe syntax, the `WHERE` operator also replaces the
[`HAVING` clause](query-syntax.md#having_clause) and [`QUALIFY` clause](query-syntax.md#qualify_clause) in
standard syntax. For example, after performing aggregation with the
[`AGGREGATE` operator](#aggregate_pipe_operator), use the `WHERE` operator
instead of the `HAVING` clause. For [window functions](window-function-calls.md) inside
...
[`GROUP BY` clause](query-syntax.md#group_by_clause) or a `SELECT` list with
[aggregate functions](aggregate_functions.md) or both. In pipe syntax, the
...
using the [`AS` operator](#as_pipe_operator). Window
functions aren't allowed, but the [`EXTEND` operator](#extend_pipe_operator) can
...
equivalent to applying the [`ORDER BY` operator](#order_by_pipe_operator) as part
...
to using a [`SELECT DISTINCT` clause](query-syntax.md#select_distinct) or
[`UNION DISTINCT` clause](query-syntax.md#union) in standard syntax, but the `DISTINCT`
...
[`JOIN` operation](query-syntax.md#join_types) in standard syntax. The input table is the
...
[`ON` expression](query-syntax.md#on_clause), then an alias can be added using the
[`AS` operator](#as_pipe_operator) before the `JOIN` arguments.
...
[table function calls](query-syntax.md#table_function_calls) in standard syntax.
...
argument** and omitted from the argument list.
...
as the [`ORDER BY` clause](query-syntax.md#order_by_clause) in standard syntax. Suffixes like
`DESC` and `NULLS LAST` are supported for
customizing the ordering for each expression.

In pipe syntax, the [`AGGREGATE` operator](#aggregate_pipe_operator) also
supports [shorthand ordering suffixes](#shorthand_order_pipe_syntax) to
...
clause](query-syntax.md#limit_and_offset_clause) in standard syntax.
...
[`UNION` set operator](query-syntax.md#union) in standard syntax. However, in pipe
...
[`INTERSECT` set operator](query-syntax.md#intersect) in standard syntax. However, in
...
[`EXCEPT` set operator](query-syntax.md#except) in standard syntax. However, in pipe
...
operator behaves the same as [`TABLESAMPLE` operator](query-syntax.md#tablesample_operator) in
standard syntax.
...
can reference, similar to standard [`WITH` clauses](query-syntax.md#with_clause). Ignores the
...
[`PIVOT` operator](query-syntax.md#pivot_operator) in standard syntax.

The output table has one pivot column for each value provided in the list.
Each pivot column has an explicit name if provided, or a string
representation of `value` if not. If `prefix` is specified, the prefix is
appended to the `pivot_column` name with an underscore in the format
`prefix_pivot_column`.

Specify `table_alias` to assign an alias to the result table produced by the
pivot operation.
...
The following example includes `pivot_column` aliases (`first_quarter` and
`second_quarter`) for the rotated columns and a `table_alias` (`pivoted_sales`)
for the result table. The subsequent `SELECT` operator references the table
...
[`UNPIVOT` operator](query-syntax.md#unpivot_operator) in standard syntax.
...
same as the
[`MATCH_RECOGNIZE` clause](query-syntax.md#match_recognize_clause) in standard syntax.
