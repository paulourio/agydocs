...
The `INFORMATION_SCHEMA.WRITE_API_TIMELINE` view contains per minute
aggregated BigQuery Storage Write API (gRPC) ingestion statistics for the current project.
...
BigQuery that uses the BigQuery Storage Write API (gRPC). See [BigQuery Storage Write API (gRPC)](https://docs.cloud.google.com/bigquery/docs/write-api) for more information.
...
To query the `INFORMATION_SCHEMA.WRITE_API_TIMELINE` view, you need
...
Queries against this view must include a [region qualifier](information-schema-intro.md#syntax).
...
|---|---|---|
| ``[PROJECT_ID.]`region-REGION`.INFORMATION_SCHEMA.WRITE_API_TIMELINE[_BY_PROJECT]`` | Project level | <var translate="no">REGION</var> |
...
requests for all tables in the project in the last 30 minutes, split by stream
type and error code:
...
The following example calculates a per minute breakdown of successful and
failed append requests, split into error code categories. This query could be
...
The following example returns the BigQuery Storage Write API (gRPC) ingestion statistics
for the 10 tables with the most incoming traffic:
...
The following example calculates a per-day breakdown of errors for a specific
table, split by error code:
