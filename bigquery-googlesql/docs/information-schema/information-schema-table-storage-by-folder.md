...
The `INFORMATION_SCHEMA.TABLE_STORAGE_BY_FOLDER` view contains one row for each
...
a day to appear in the `INFORMATION_SCHEMA.TABLE_STORAGE` view. If a dataset
containing more than 1,000 tables is deleted, this view won't reflect the change
until the [time travel window](https://docs.cloud.google.com/bigquery/docs/time-travel#time_travel) for the
...
view](information-schema-table-storage-usage.md).
...
To query the `INFORMATION_SCHEMA.TABLE_STORAGE_BY_FOLDER` view, you need the
...
[region qualifier](information-schema-intro.md#syntax). The
...
The following query shows which projects in a folder use
the most storage:
