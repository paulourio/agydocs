...
| `data_type` | `STRING` | The column's GoogleSQL [data type](../standard-sql/data-types.md). |
| `description` | `STRING` | The column's description. |
| `collation_name` | `STRING` | The name of the [collation specification](../standard-sql/collation-concepts.md) if it exists; otherwise, `NULL`. If a `STRING`, `ARRAY<STRING>`, or `STRING` field in a `STRUCT` is passed in, the collation specification is returned if it exists; otherwise, `NULL` is returned. |
