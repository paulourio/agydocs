...
The `INFORMATION_SCHEMA.STREAMING_TIMELINE` view contains per minute
...
To query the `INFORMATION_SCHEMA.STREAMING_TIMELINE` view, you need
...
|---|---|---|
| ``[PROJECT_ID.]`region-REGION`.INFORMATION_SCHEMA.STREAMING_TIMELINE[_BY_PROJECT]`` | Project level | <var translate="no">REGION</var> |
...
requests for all tables in the project in the last 30 minutes, split by error
code:
...
failed streaming requests, split into error code categories. This query could be
...
The following example returns the streaming statistics for the 10 tables with
the most incoming traffic:
