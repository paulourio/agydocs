...
-- Overwriting existing partitions with delimited CSV files
...
When reading external directories arranged by directory key paths (`/year=2026/month=03/`), declare partition columns directly within the statement. The query engine maps directory tokens to typed partition columns during data loads:
...
The `EXPORT DATA` statement streams query results directly into Cloud Storage objects in Parquet, CSV, JSON, or Avro formats.

### 2.1 Parquet Export with Compression
...
- **Inspect Status:** Check active execution state via `INFORMATION_SCHEMA.CONTINUOUS_QUERIES`.
...
### 4.2 Dynamic Parameter Substitution
