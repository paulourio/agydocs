# [`gcloud topic projections`](topic-projections.md)

## NAME

    [`gcloud topic projections`](topic-projections.md) - resource projections supplementary help
...
## DESCRIPTION
    ...
    details run [`gcloud topic formats`](topic-formats.md).

    Use the --filter flag to select resources to be listed. For details run $
    [`gcloud topic filters`](topic-filters.md).

### Use resource-keys to reach resource items through a unique path of names

    from the root. For details run [`gcloud topic resource-keys`](topic-resource-keys.md).
    ...
    The builtin transform functions are:
     ...
     if(expr)
        ...
        The arguments are:
         ...
         expr
            A command flag filter name expression. See [`gcloud topic filters`](topic-filters.md) for
